//
//  SimpleBuyPaymentAccountPatcher.swift
//  PlatformKit
//
...
/// SimpleBuyPaymentAccountPatcher goal is to 'fix' PaymentAccount objects that have missing fields.
...
final class SimpleBuyPaymentAccountPatcher {
    static let targetBankCode = "LHVBEE22"
    static let targetBankCountry = "Estonia"

    func patch(_ account: SimpleBuyPaymentAccount, recipientName: String) -> SimpleBuyPaymentAccount {
    ...
    private func patchEuroPaymentAccount(_ account: SimpleBuyPaymentAccount, recipientName: String) -> SimpleBuyPaymentAccount {
        /// Account must be a SimpleBuyPaymentAccountEUR. If not, don't patch it.
        guard let eurAccount = account as? SimpleBuyPaymentAccountEUR else {
            return account
        }
