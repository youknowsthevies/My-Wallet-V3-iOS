...
open class Router<InteractorType>: Routing {
    ...
    /// Called immediately before the router is loading.
    ///
    /// This method is invoked only once. Subclasses should override this method to perform one time setup logic,
    /// such as preparing for loading. e.g - binding to a `PublishRelay` derived stream of events. The default implementation does nothing.
    open func willLoad() {
        // No-op
    }
    ...
    private func internalWillLoad() {
        lifecycleSubject.onNext(.willLoad)
    }
