...
private struct Line: Shape {
    ...
    /// Sliding average of data using chunk size
    private var smoothedData: [Double] {
        // stride + map can be replaced by swift-algorithms `chunks(ofCount: smoothingChunkSize)`
        stride(from: 0, to: data.count, by: smoothingChunkSize)
            .map {
                data[$0..<min($0 + smoothingChunkSize, data.count)]
            }
    ...
    func path(in rect: CGRect) -> Path {
        let inset = rect.inset(by: UIEdgeInsets(top: 0, left: 0, bottom: 48, right: 0))

        var path: Path
        if isSmooth, smoothingChunkSize > 1 {
            let points = smoothedData.points(in: inset)
            path = smoothedPath(points)
        } else {
            let points = data.points(in: inset)
            path = Path()
...
extension Array where Element == Double {
    func points(in rect: CGRect) -> [CGPoint] {
        ...
        let x = rect.width / CGFloat(count - 1)
        let diff = max - min
        return enumerated().map { index, element in
            let percentage = (element - min) / diff
            return CGPoint(
                x: x * CGFloat(index),
                y: rect.height - CGFloat(percentage) * rect.height
            )
        }
    }
}
